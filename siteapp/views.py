        ...
        if 'action' in request.POST:
            ...
            if Professor.objects.filter(email=email).exists():
                ...
                if professor1 and professor1.id != professor.id:
                    messages.error(request, f'Já existe professor cadastrado com o email {email}.')
                    return redirect('configuracoes/')
            ...
            if (senha != user.password and senha != ""):
                ...
                user.save()
            return HttpResponseRedirect('configuracoes/')
...
def matricula(request):
    ...
    if request.method == 'POST':
        ...
        if Aluno.objects.filter(cpf=cpf).exists():
            messages.error(request, f'Já existe aluno cadastrado com o CPF {cpf}.')
            return redirect('matricula')
        
        if Aluno.objects.filter(rg=rg).exists():
            messages.error(request, f'Já existe aluno cadastrado com o RG {rg}.')
            return redirect('matricula')
        
        if Aluno.objects.filter(email=email).exists():
            messages.error(request, f'Já existe aluno cadastrado com o email {email}.')
            return redirect('matricula')
...
def contratar(request):
    if request.method == 'POST':
        ...
        if disc == "Artes" and ano != '1':
            messages.error(request, 'Só existe a matéria Artes pro 1° ano do ensino médio.')
        ...
        if Professor.objects.filter(cpf=cpf).exists():
            messages.error(request, f'Já existe professor cadastrado com o CPF {cpf}.')
            return redirect('contratar')
        
        if Professor.objects.filter(rg=rg).exists():
            messages.error(request, f'Já existe professor cadastrado com o RG {rg}.')
            return redirect('contratar')
        ...
        if Professor.objects.filter(disciplina=disciplina).exists():
            messages.error(request, f'Já existe professor para a disciplina de {disc}.')
            return redirect('contratar')
        ...
        return HttpResponseRedirect('main/')
    return render(request, 'adicionaProfessor.html')
